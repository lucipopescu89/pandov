...
- **The statement** is stored one line to an entry, and every screen keeps those lines. The author made it smaller and airier on 2026-09-25:
  ...
  - lines 2.5 apart;
  - 230 canvas units under the menu instead of the design's 146.7;
...
All of it lives at the top of `components/pendant-viewer.tsx`: `LOOKS`, `STUDIO_TURN`, `STUDIO_INTENSITY`, `GRAIN_TILE` and `GRAIN_STRENGTH`. Every pendant page renders through that one viewer, so a new pendant gets the metals without anything being set for it. Don't tune a metal for one pendant: a change is a change to all seven, and it is the author's to make.
