...
### The Second Wind pendant pages (2026-09-22)

Each of the seven pendants has a page, `/body/second-wind/<slug>`, opened by clicking its photograph on Second Wind (`PENDANT_PAGES` in `body-presentation.tsx`; the link is laid over the photograph by reading the export's own pattern rect). **They are one template and seven entries of data.** `app/body/second-wind/[pendant]/page.tsx` renders `components/pendant-page.tsx` for each entry in `lib/second-wind.ts`: name, the two lines, height, photographs, model, prices. A design change goes in `pendant-page.tsx` / `pendant-gallery.tsx` and reaches all seven; a price or a photograph goes in `lib/second-wind.ts` and reaches one.

The layout is the author's Figma frame (1920 × 1794), lengths in canvas units through `lib/canvas-length.ts`. The photograph and the model share one frame on the left. Beside it is a column of marks: a grey ring per photograph, and a gold one four steps lower for the model. Then come the text and prices behind a hairline, which stay on screen (sticky) while the photograph scrolls. A click on a photograph fades to the next; the model is reached only from its mark, and drawn at 35% of the size that would fill the frame. Only the photograph shown and the next are fetched, and three.js only when the model is asked for. The phone layout (stacked) is a placeholder the author will design. **Every pendant shows Icarus's prices as a placeholder** (`PLACEHOLDER_PRICES`) until the author gives each its own.

The photographs come from `D:\PANDOV\2_BODY\1_SECOND_WIND\WEBSITE\<PENDANT>\` and are encoded into `public/images/body/<folder>/<folder>-<n>[-phone].avif`, 1440px and 1080px wide, AVIF q90 — or q70 for those shot on textured stone, whose grain doubles the bytes at q90 with no difference visible at the 753px they are drawn.
