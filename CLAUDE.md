...
**Two faces, by length of text.** Julius Sans One is the display face — logo, menus, titles, captions, any short line. **Assistant** is the face for longer running text — paragraphs, descriptions, anything that is read rather than glanced at. Never set a paragraph in Julius Sans One. *(Assistant is loaded on the pendant pages only, weight 300, since 2026-09-23: through `next/font` in `components/pendant-page.tsx`, as the CSS variable `--font-body`, so only the pages that use it download it. No other page sets running text in it yet.)*
