...
- **The name** is also at the menu's size, 12px, on every screen, since the same day. It was 44px in the design. The line under it is now the larger of the two, in Assistant; the author asked only for the name.
...
- **The order panel** (`pendant-order.tsx`). Six metals sit in rows of two, each row followed by the span of its prices, and one is chosen, Brass at first. Brass and Silver open a Finish row (Natural / Polished) and Gold a Karat row (14K / 18K); the others open none. The row fades and eases open or shut (`grid-template-rows` 0fr ↔ 1fr), so the price below it never jumps, and changing the metal resets it to its first option. The price fades in beside the choice in words.
- **Prices live in one place**, `PRICES` in `lib/second-wind.ts`, laid out as the panel shows them. They are Icarus's, from the design. **Every pendant shows them as a placeholder** until the author gives each its own.
...
- **Silver.** Natural (0.95, 0.94, 0.91) at roughness 0.10 with a fifth of the grain. The author found Sculpteo's Polished, which is our Natural, flat and grey, and asked for it nearer their Mirror Polished. Polished (0.97, 0.96, 0.92) at roughness 0.05.
