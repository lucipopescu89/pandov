...
On 2026-09-19 each one was decoded into a sibling file and the `<image xlink:href>` repointed at it (`icarus.svg` → `/images/body/icarus-pendant.png`), leaving the export otherwise byte-for-byte as Figma wrote it. The document went from 15MB to 1.6MB, and the photographs became ordinary cacheable files like every other image on the site.

**A fresh export from Figma will arrive with the photograph embedded again.** Lift it out before committing: decode the one `data:image/png;base64` value into `<name>-pendant.png` beside the SVG and leave `/images/body/<name>-pendant.png` in its place. Don't resize it — the `<image width/height>` in the SVG is the photograph's own pixel size, and the two are expected to match.
