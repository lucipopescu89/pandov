...
- **The statement** is stored one line to an entry, where it breaks on a computer; a phone rewraps it (`text-wrap: balance`).
- **The gallery** (`pendant-gallery.tsx`). A thumbnail crossfades the frame to its photograph. The active one is at full strength inside a 0.5px hairline; the rest are at 45%. The last thumbnail is the pendant's cut-out marked "3D", and it puts the model in the frame, at 35% of the size that would fill it. A photograph is fetched only when it is shown, next in line, or under the pointer, and the frame changes to it only once it has arrived. three.js loads only when the model is asked for.
